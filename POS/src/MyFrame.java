...
public class MyFrame extends JFrame implements ActionListener {
	...
	Ingredients ing = Ingredients.getInstance();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonCompleteOrder) {
			...
			Ingredients ing = Ingredients.getInstance();
			ing.updateDatabse();
			String fileName = "table" + tableNum + "Csv.csv";

			try {
				FileWriter writer = new FileWriter(fileName, true);
				Iterator<Item> enu = order.iterator();
				while (enu.hasNext()) {
					Item item = enu.next();
					String data = item.getId() + ";" + item.getName() + ";" + item.getPrice() + "\n";
					writer.append(data);
				}
				writer.close();
		...
		if (e.getSource() == buttonOrderBurger) {
			...
			String meatType = "meat";
			meatType = cbBurgerSingleBeef.isSelected() ? "SingleBeef" : meatType;
			meatType = cbBurgerDoubleBeef.isSelected() ? "DoubleBeef" : meatType;
			meatType = cbBurgerSingleChicken.isSelected() ? "SingleChicken" : meatType;
			meatType = cbBurgerDoubleChicken.isSelected() ? "DoubleChicken" : meatType;

			String breadType = cbBurgerPlainBread.isSelected() ? "PlainBread" : "SesameBread";

			Burger b = new Burger(meatType + ";" + breadType, 20, breadType, meatType);
			b.setId(orderCounter++);
			order.add(b);
		}
		...
		if (e.getSource() == buttonOrderPizza) {
			...
			volume = cbPizzaSmall.isSelected() ? "Small" : volume;
			volume = cbPizzaMedium.isSelected() ? "Medium" : volume;
			volume = cbPizzaLarge.isSelected() ? "Large" : volume;

			String doughType = cbPizzaFlatBread.isSelected() ? "FlatBread" : "YeastedBread";

			Pizza b = new Pizza(doughType + ";" + volume, 20, doughType, volume);
			b.setId(orderCounter++);
			order.add(b);
		}
		...
		if (e.getSource() == buttonOrderPasta) {
			...
			String pastaType = "pasta";
			pastaType = cbPastaFarfalle.isSelected() ? "Farfalle" : pastaType;
			pastaType = cbPastaFusilli.isSelected() ? "Fusilli" : pastaType;
			pastaType = cbPastaSpaghetti.isSelected() ? "Spaghetti" : pastaType;

			String sauceType = cbPastaAlfredoSauce.isSelected() ? "AlfredoSauce" : "TomatoSauce";

			Pasta b = new Pasta(pastaType + ";" + sauceType, 20, sauceType, pastaType);
			b.setId(orderCounter++);
			order.add(b);
		...
		if (e.getSource() == buttonOrderBeer) {
			...
			String brandType = cbTuborug.isSelected() ? "Tuborug" : "Karlsberg";
			brandType = cbEfeys.isSelected() ? "Efeys" : "Karlsberg";

			String glassType = cbGlass.isSelected() ? "Glass" : "Can";

			Double volume = cb33Cl.isSelected() ? 0.33 : 0.5;

			Beer b = new Beer(brandType + ";" + glassType + ";" + volume, 30, brandType, glassType, volume);
			b.setId(orderCounter++);
			order.add(b);
		...
		if (e.getSource() == buttonOrderCocktail) {
			...
			String Vodka = cbVodka.isSelected() ? "Vodka" : "";
			String Gin = cbGin.isSelected() ? "Gin" : "";
			String Tequila = cbTequila.isSelected() ? "Tequila" : "";
			String Rom = cbRom.isSelected() ? "Rom" : "";
			String Lime = cbLime.isSelected() ? "Lime" : "";

			String glassType = cbShortGlass.isSelected() ? "ShortGlass" : "LongGlass";

			String cocktailName = Vodka + ";" + Gin + ";" + Tequila + ";" + Rom + ";" + Lime + ";" + glassType;

			Cocktail b = new Cocktail(cocktailName, 20, glassType);
			b.setId(orderCounter++);
			order.add(b);
		...
		if (e.getSource() == buttonOrderWine) {
			...
			String brandType = "wine";
			brandType = cbDolucaRed.isSelected() ? "DolucaRed" : brandType;
			brandType = cbBuzbagRed.isSelected() ? "BuzbagRed" : brandType;
			brandType = cbBuzbagRose.isSelected() ? "BuzbagRose" : brandType;

			String glassType = cbOneGlass.isSelected() ? "OneGlass" : "Bottle";

			Wine b = new Wine(brandType + ";" + glassType, 20, brandType, glassType);
			b.setId(orderCounter++);
			order.add(b);
